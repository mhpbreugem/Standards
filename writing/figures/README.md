...
- **No overlapping text.** No text element may overlap any other text or any
  ...
  **Never clip a curve or bar, and never shrink the data, to make room for text.**
  ...
  - **Widening recipe (only for unbounded y).** When the legend overlaps
    a curve and the y-axis is not a bounded quantity:
      1. First try moving the legend to a sparser corner.
      2. If still tight, widen the axis range by at least 25% above the
         data maximum (or below the minimum) so the legend sits in a
         clearly empty band.
      3. Never clip a curve, never shrink the data, never overlap text.
    For bounded y-axes, see the **Bounded quantities** subsection below —
    widening is forbidden there.
...
- [ ] Vector PDF from pgfplots, 8 cm square axes (or up to 16 cm × 7 cm
      for bar charts with at least 6 categories), document serif fonts
- [ ] Curves use the four ordered styles (bcone / bctwo / bcthree / bccara)
- [ ] Linear interpolation (no `smooth` unless at least 40 dense points
      and the function is known smooth analytically)
...
- [ ] If the y-axis is bounded (probability, CDF, percentage, R²), the
      axis range stays inside that bound — legend space is found inside
      the data window or outside the axis, never by inflating ymax
- [ ] Log axes with all ticks in [0.001, 10000] use decimal labels via
      `log ticks with fixed point`; outside that range, keep the default
      10^x notation
- [ ] Each log decade carries at least 3 tick labels in a 1-2-5
      multiplicative sequence, specified via explicit `xtick={...}`
- [ ] If the figure is part of a paired sweep (other panel sweeps a
      parameter fixed in this one), a thin gray vertical guide marks the
      fixed value, inserted as the first `\addplot` so it sits behind the
      data
- [ ] Contour plots use five close-spaced levels centred on the value of
      interest, with grayscale-filled bands between adjacent contour
      lines (darkest at centre, symmetric) and crisp contour lines drawn
      on top
- [ ] Bar charts: aspect chosen so category ticks do not overlap; value
      labels via `nodes near coords` with 3-decimal precision and fixed
      zerofill; bar separation widened (`ybar=6pt`+) before reaching for
      rotated labels; zero-height bars and their labels both omitted
- [ ] Legend `draw=none, fill=none`, placed in the emptiest corner; if
      still tight, widen axis by at least 25% (only for unbounded y) or
